...
const ChartTwo: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Ensure that the data is retrieved correctly
        const shortlistedData = getAllShortListed || [];
        const rejectedData = getAllRejected || [];
        const applicantsData = getFiles || [];

        const processedData = processData(shortlistedData, rejectedData, applicantsData);
  ...
  const processData = (shortlistedData: any[], rejectedData: any[], applicantsData: any[]) => {
    const weeklyData = processDataByInterval(applicantsData, filter);
    const shortlistedByInterval = processDataByInterval(shortlistedData, filter);
    const rejectedByInterval = processDataByInterval(rejectedData, filter);

    const categories = Object.keys(weeklyData);

    const series = [
      { name: "Applicants", data: categories.map(week => weeklyData[week] || 0) },
      { name: "Shortlisted", data: categories.map(week => shortlistedByInterval[week] || 0) },
      { name: "Not Shortlisted", data: categories.map(week => rejectedByInterval[week] || 0) },
    ];
